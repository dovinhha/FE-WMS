...
const CustomerInfor = ({ handleChangeTab }) => {
  ...
  const dispatch = useDispatch();
  const { orderId, customerOrganizationId } = useParams();
  ...
  const [queryCustomer, setQueryCustomer] = useState({
    status: "active",
    // populate: "customerOrganizationId",
  });
  const [queryCustomerId, setQueryCustomerId] = useState({
    status: "active",
    populate: "childrentIds",
  });
  const [queryCustomerInOrders, setQueryCustomerInOrders] = useState({
    page: 1,
    limit: 99999,
  });
  ...
  useEffect(() => {
    if (!_.isEmpty(customerOrganizationId)) {
      setCustomerValue({
        ...
        label: queryString.parse(search.split("?")[1]).customerOrganizationName,
      });
      handleGetCustomerById(customerOrganizationId);
    }
  ...
  useEffect(() => {
    if (!_.isEmpty(orderById)) {
      ...
      orderById.products.forEach((item) => {
        temp.push({ ...item, amountInput: 0, contactPerson: "", phone: "" });
      });
      setProducts(temp);
    }
  }, [orderById]);
          ...
          <Col md={12}>
            <Table>
              <thead>
                ...
                >
                  Đơn vị <Select isDisabled={true} value={customerValue} />
                </th>
